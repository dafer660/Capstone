...
export class Actors extends Component {
    ...
    componentDidMount() {
        if (this.state.allActors.length <= 0) {
            this.getActors()
        }
    }

    componentDidUpdate(prevProps, prevState, snapshot) {
        if (this.state.totalActors === 0) {
            this.getActors()
        }
    ...
    getActors = async () => {
        ...
        if (permissions) {
            fetch(`http://localhost:5000/actors?page=${this.state.currentPage}&limit=${10}&offset=${this.state.offset}`, {
                method: 'GET',
                headers: {
                    Authorization: 'Bearer ' + sessionStorage.getItem('token')
                }
